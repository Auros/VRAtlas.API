...
public class CrosspostingService : ICrosspostingService
{
    ...
    public async Task<Group> GetCrossposterGroupAsync(CrosspostSource source)
    {
        ...
        var description = (source.Description ?? string.Empty) + $"\n\nThis group has been auto-generated by VR Atlas. It is not managed by {source.Name}.";
        if (group is null)
            group = await _groupService.CreateGroupAsync(source.Name, description, source.Icon, source.Banner, null, source.Source.ToString());
        else
            group = await _groupService.ModifyGroupAsync(group.Id, description, source.Icon, source.Banner);
